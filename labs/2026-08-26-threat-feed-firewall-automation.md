# Threat-Feed-Driven Security Automation (Firewall Blocking + Malware Removal)

**Domain:** Security Operations — Automation & Orchestration
...
3. Identified which hashing algorithm the script used to compare files
   against the feed as part of reviewing it — **SHA-256**. That matters:
...
activity — phishing, ransomware, botnet command-and-control, DDoS
...
a name in the field: **DNS sinkholing** — pointing a known-bad domain to a
harmless address (usually loopback) instead of letting it resolve normally.
...
- The lab's own closing note is worth keeping: this only protects the one
  ...
  — a DNS server-side block (e.g., a DNS Response Policy Zone) or a
  network-wide DNS filtering service (Pi-hole, Cisco Umbrella, Quad9, etc.)
  — rather than editing a hosts file on every endpoint individually.
