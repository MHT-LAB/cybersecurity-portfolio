...
<!-- LAB-INDEX-START -->
- [LFI/RFI Exploitation and Log-Based Threat Hunting](https://github.com/MHT-LAB/cybersecurity-portfolio/blob/master/labs/2026-09-19-lfi-rfi-exploitation-and-threat-hunting.md): Exploited local and remote file inclusion on DVWA (directory traversal, command injection recon, session hijack via RFI+XSS), then threat-hunted the same attack chain through Apache access logs.
- [Reflected XSS: Exploiting DVWA and Investigating the Attack in Apache Logs](https://github.com/MHT-LAB/cybersecurity-portfolio/blob/master/labs/2026-09-19-reflected-xss-attack-and-log-investigation.md): Exploited a reflected XSS vulnerability to hijack a session cookie via a phishing-style redirect, then investigated the same attack from the Apache access log, decoding percent-encoded payloads and reconstructing the attack sequence via referrer chains.
